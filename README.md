...
- **Tauri + React** — 基于 Tauri 2.x 的跨平台桌面应用 + React/TypeScript 前端
- **CLI 命令行** — clap 驱动的命令行工具，适合脚本集成
- **轻量级** — 使用系统 WebView，SQLite 内嵌数据库，无需外部依赖
- **智能分类** — 分组管理、标签系统、环境标识（dev/staging/prod）
...
apikey-vault change-password                   # 修改主密码
...
apikey-vault key test openai-api               # 测试密钥连通性
...
# === 标签管理 ===
apikey-vault tag list                          # 列出所有标签
apikey-vault tag add openai-api "ai"           # 添加标签
apikey-vault tag remove openai-api "ai"        # 移除标签

# === 搜索 ===
apikey-vault search "openai"                   # 搜索密钥
apikey-vault search "openai" --group <id>      # 按分组过滤
apikey-vault search "openai" --tag "ai"        # 按标签过滤
...
apikey-vault config reset                      # 重置为默认配置
```
...
# 或使用编译后的二进制
./target/release/API Key Vault.exe   # Windows
./target/release/API Key Vault       # macOS/Linux
```
...
配置文件路径：`{用户配置目录}/apikey-vault/config.toml`
