...
impl Default for KeyEditForm {
    ...
    fn default() -> Self {
        ...
        Self {
            ...
            tags: Vec::new(),                       // 绌烘爣绛惧垪琛╜r
            tags_str: String::new(),               // 空标签字符串
...
impl KeyEditForm {
    ...
    fn from_entry(entry: &KeyEntry, _vault: &Vault) -> Self {
        ...
        Self {
            ...
            tags: entry.tags.clone(),               // 鐩存帴浠庢潯鐩鍒舵爣绛惧垪琛╜r
            tags_str: entry.tags.join(", "),      // 标签连接为字符串
    ...
    fn validate(&mut self, is_new: bool) -> bool {
...
pub struct VaultApp {
    ...
    tag_dialog_open: bool,         // 标签输入弹窗是否打开
...
impl VaultApp {
    ...
    // show_tag_dialog() - 显示添加标签的弹窗

    // 使用 egui::Window 实现模态窗口，包含输入框 + 确定/取消按钮
    ...
    fn show_tag_dialog(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("添加标签")
            ...
            .fixed_size(Vec2::new(300.0, 130.0))

            .show(ctx, |ui| {

                ui.add_space(12.0);

                ui.label(RichText::new("输入标签名称").size(13.0).color(theme.text_secondary));

                ui.add_space(8.0);

                let resp = ui.add(

                    egui::TextEdit::singleline(&mut self.tag_dialog_input)
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.add(
                        ...
                        egui::Button::new(RichText::new("取消").size(13.0).color(theme.text_secondary))
                    ...
                    if ui.add(
                        ...
                        egui::Button::new(RichText::new("确定").size(13.0).color(Color32::WHITE))
                        ...
                        if !self.tag_dialog_input.is_empty() {
        ...
        if close {
            ...
            if confirmed {
                ...
                if self.edit_form.tags.contains(&self.tag_dialog_input) {

                    self.add_notification(Notification::error("标签已存在"));

                } else {

                    self.edit_form.tags.push(self.tag_dialog_input.clone());

                }

                self.tag_dialog_input.clear();
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.vertical(|ui| {
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    let tags_clone = self.edit_form.tags.clone();
                                    for (i, tag) in tags_clone.iter().enumerate() {
                                        // 标签小方块
                                        egui::Frame::none()
                                            ...
                                            .show(ui, |ui| {
                                                ui.label(RichText::new(tag).size(11.0).color(theme.accent));
                                            });
                                        // X 删除按钮
                                        if ui.add(
                                    ...
                                    if let Some(idx) = remove_idx {
                                        self.edit_form.tags.remove(idx);
                                    }

                                    // 添加标签按钮
                                    if ui.add(
                                        egui::Button::new(RichText::new("＋").size(16.0).color(theme.accent))
                                            .fill(Color32::TRANSPARENT)
                                            .frame(false)
                                    ).clicked() {
                                        self.tag_dialog_open = !self.tag_dialog_open;
                                    }

                                });

                                // 标签输入弹窗（由 update() 中的 show_tag_dialog 渲染）
                                // （此处仅预留渲染位置，实际弹窗由独立的 Window 实现）
    ...
    fn save_key_form(&mut self, index: Option<usize>) {
        ...
        if !self.edit_form.validate(self.edit_is_new) {
        ...
        let tags: Vec<String> = self.edit_form.tags.clone();
...
match self.vault.create_group(self.new_group_name.clone()) {
    ...
    fn do_import(&mut self) {
        ...
        let records = match self.import_format.as_str() {
            ...
            "csv" => crate::import_export::parse_csv(&content).unwrap_or_default(),

            "json" => crate::import_export::parse_json(&content).unwrap_or_default(),

            "dotenv" => crate::import_export::parse_dotenv(&content).unwrap_or_default(),
    ...
    fn do_export(&mut self) {
        ...
        // 使用 import_export 模块的统一导出函数（不包含密钥值）
        ...
        let output = match self.export_format.as_str() {
            "json" => crate::import_export::export_json_str(&export_data).unwrap_or_default(),
            "csv" => crate::import_export::export_csv(&export_data).unwrap_or_default(),
            "dotenv" => crate::import_export::export_dotenv_str(&export_data).unwrap_or_default(),
            _ => String::new(),
        };
